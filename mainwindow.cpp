...
void MainWindow::down() {
    ...
    for (int j = 0; j < 4; ++j)
    {
        for (int i = 3; i >= 0; --i)
        {
            if (labels[i][j]->text() == "") // Если текущая ячейка пуста
            {
                ...
                {
                    if (labels[k][j]->text() != "")
                    { // Поиск первой непустой ячейки выше текущей
    ...
    for (int j = 0; j < 4; ++j) {
        for (int i = 2; i >= 0; i--)
        {
            ...
            { // Если две соседние ячейки имеют одинаковые значения и они не пусты
